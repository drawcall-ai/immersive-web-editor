...
const bundledEntry = new URL('authoring-api.js', tmpDir);
const bundledServerEntry = new URL('server.js', tmpDir);
...
test('custom field component names stay plain data for the editor', () => {
  const plugin = editorPlugin();
  const source = `import { config, defineField, val } from 'immersive-web-editor';
const custom = defineField({
  defaultValue: "hello",
  component: 'example.mood',
});
const object = config('Custom', {
  title: val("hello", custom)
});`;

  const file = fileURLToPath(new URL('../../../examples/ai-chat/src/custom-fields.tsx', import.meta.url));
  const result = plugin.transform(source, file);

  assert.match(result.code, /component:\s*'example\.mood'/);
  assert.equal(result.code.includes('custom-field-components'), false);
});
